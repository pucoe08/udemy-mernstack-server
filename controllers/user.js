...
export const login = asyncError(async (req, res, next) => {
    ...
    const user = await User.findOne({ email }).select("+password");  //req. bcz in schema 'selected':false
    if (!user) {
    ...
    // comaprePassword method is written in  in user model.
    const isMatched = await user.comparePassword(password);
    if (!isMatched) {
        // 'next' will  the next middleware in app.js
        return next(new ErrorHandler("Incorrect Email or Password", 400));
    }
...
export const forgetPassword = asyncError(async (req, res, next) => {
    ...
    // OTP Generation Starts
    //formula to generate random number b/w (max,min) eg. (2000,10000)
    // max.random()*(max-min)+min
    ...
    try {
        await sendEmail("OTP for resetting the Password", user.email, message);
    } catch (error) {
        user.otp = null;
...
export const resetPassword = asyncError(async (req, res, next) => {
    ...
    const user = await User.findOne({
        otp,
        otp_expire: {
            $gt: Date.now() // expire time is greater than current time than only user can be fetched '$gt' basics of mongoDb 
        }
    });
